...
public class AdminAddPermissionControllers extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public AdminAddPermissionControllers() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		int idquyen = 0;
		try {
			idquyen = Integer.parseInt(request.getParameter("addquyen"));
		} catch (Exception e) {
		...
		CatUser catUer = new CatUser(idquyen, addquyen, true, true);
		int add = catUserDao.addquyen(catUer);
		if(add > 0) {
